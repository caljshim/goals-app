...
def test_ensure_schema_adds_important_columns_to_old_tables(postgres_engine_factory):
    """ensure_schema() must add the new `important` column to reminder and goal
    tables that predate the feature, defaulting existing rows to False."""
    eng = postgres_engine_factory()
    with eng.begin() as conn:
        conn.exec_driver_sql(
            'CREATE TABLE "reminder" ('
            "id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
            "title VARCHAR NOT NULL, scheduled_for DATE NOT NULL, "
            "reminder_time VARCHAR, notes VARCHAR, completed BOOLEAN DEFAULT FALSE)"
        )
        conn.exec_driver_sql(
            'INSERT INTO "reminder" (title, scheduled_for) '
            "VALUES ('Pay rent', '2026-07-31')"
        )
        conn.exec_driver_sql(
            'CREATE TABLE "goal" ('
            "id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
    ...
    SQLModel.metadata.create_all(eng)  # builds goalhistory etc.; skips existing goal
    from app.budget.db import ensure_schema, _column_names
    ensure_schema(eng)
    with eng.connect() as conn:
        assert "important" in _column_names(conn, "reminder")
        assert "important" in _column_names(conn, "goal")
        row = conn.exec_driver_sql('SELECT important FROM "reminder"').first()
