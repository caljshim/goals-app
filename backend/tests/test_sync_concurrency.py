...
def test_concurrent_syncs_do_not_crash_or_duplicate(engine, monkeypatch):
    """Two overlapping /plaid/sync requests (auto-sync + a manual button) run in
    FastAPI's threadpool on separate PostgreSQL connections and both ingest the same
    Plaid transaction. Without serialization, _upsert's check-then-insert races and
    the loser hits the unique plaid_transaction_id constraint on
